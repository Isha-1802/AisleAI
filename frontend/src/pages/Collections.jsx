...
function Collections() {
    ...
    useEffect(() => {
        loadProducts();
    }, [page, searchParams]); // correct + stable
    ...
    const loadProducts = async () => {
        ...
        try {
            ...
            if (page === 1) setProducts(response.data.products);
                ...
                <aside className="filters-sidebar-clean">
                    ...
                    <div className="filter-section">
                        ...
                        <div className="filter-options">
                            ...
                            {filters.brands?.slice(0, 20).map(brand => (
                                <label key={brand} className="filter-checkbox">
                                    <input
                                        type="radio"
                                        name="brand"
...
/* ------------------------------
   PRODUCT CARD (unchanged except fixes)
------------------------------ */
function ProductCard({ product }) {
    const { addToCart } = useCart();
    const { toggleWishlist, isInWishlist } = useWishlist();
    ...
    return (
        ...
        >
            ...
            {product.rating > 0 && (
                <div className="product-rating">⭐ {product.rating} ({product.reviews || 0})</div>
            )}
        </div>
    );
}
